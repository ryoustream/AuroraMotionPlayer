...
struct FILMInterpolator::Impl {
    ...
    Ort::Env           ortEnv{ORT_LOGGING_LEVEL_WARNING, "film"};
...
bool FILMInterpolator::init(const InterpolationConfig& cfg) {
    ...
    if (cfg.backend == InferenceBackend::NCNN) {
        ...
        if (m_impl->net.load_param((cfg.modelPath + "/film.param").c_str()) != 0) return false;
        if (m_impl->net.load_model((cfg.modelPath + "/film.bin").c_str())   != 0) return false;
        m_initialized = true;
        return true;
    }
#endif
...
void FILMInterpolator::shutdown() {
#ifdef AURORA_NCNN
    m_impl->net.clear();
#endif
    m_initialized = false;
...
// ── helpers ──────────────────────────────────────────────────────────────────
static int filmPad(int v) { return ((v + 63) / 64) * 64; } // FILM needs 64-alignment

static void filmYUVtoRGB(video::VideoFramePtr f, std::vector<float>& out,
                          int& outW, int& outH)
{
    int w = f->width(), h = f->height();
    outW = filmPad(w); outH = filmPad(h);
...
static video::VideoFramePtr filmRGBtoFrame(const std::vector<float>& rgb,
...
// ── interpolate ──────────────────────────────────────────────────────────────
// FILM I/O: inputs "x0","x1","time" → output "image"
// FILM uses a single timestep scalar; recursive bisection can be done externally.
    ...
    video::VideoFramePtr f0, video::VideoFramePtr f1, float t)
{
    ...
    filmYUVtoRGB(f0, rgb0, padW, padH);
    filmYUVtoRGB(f1, rgb1, padW, padH);
    ...
    if (m_impl->backend == InferenceBackend::NCNN) {
        ...
        ncnn::Mat out;
        if (ex.extract("image", out) != 0) return nullptr;

        std::vector<float> outRGB(padW*padH*3);
        memcpy(outRGB.data(), out.data, outRGB.size()*sizeof(float));
        return filmRGBtoFrame(outRGB, f0->width(), f0->height(), padW, *f0);
    }
#endif
#ifdef AURORA_ONNX
    if (m_impl->backend == InferenceBackend::ONNX) {
        ...
        const char* inames[] = {"x0","x1","time"};
        const char* onames[] = {"image"};
        ...
        return filmRGBtoFrame(hwc, f0->width(), f0->height(), padW, *f0);
    }
#endif
    return nullptr;
}
